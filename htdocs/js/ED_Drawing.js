...
ED.Report.prototype.inDetachment = function(_angle)
{
    ...
    for (key in this.rrdArray)
    {
		...
		// Create points at start of detachment and angle
		var startPoint = new ED.Point(0,0);
		startPoint.setWithPolars(500, rrd.rotation - rrd.arc/2);
		var anglePoint = new ED.Point(0,0);
		anglePoint.setWithPolars(500, _angle * Math.PI/180);
		
		// For a total detachment return true
		if (rrd.arc * 180/Math.PI > 359)
		{
			returnValue = true;
		}
		else if (startPoint.clockwiseAngleTo(anglePoint) < rrd.arc)
		{
			returnValue = true;
		}
		else
